...
function getIndexPosition (quadkey, firstQuadkey, indexDepth) {
  if (!firstQuadkey) firstQuadkey = '0'
  if (!indexDepth) indexDepth = 22
  if (quadkey === firstQuadkey) return 0
  if (indexDepth === 1) {
    // Tile cannot be found, does not exist
    // TODO: throw error here?
    return
  }
  var prefixLen = firstQuadkey.length
  // Remove `firstQuadkey` from the start of the quadkey, and digits beyond
